...
final List<Category> mainCategories = [
  Category(name: 'شنط ظهر'),
  Category(name: 'شنط نسائية عصرية'),
  Category(name: 'شنط سفر'),
  Category(name: 'أخرى'),
];
...
class BagsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          languageProvider.translate('bags.title'),
      ...
      body: ListView(
        ...
        children: [
          _buildCategoryTile(
            context,
            languageProvider.translate('bags.backpacks'),
            Icons.backpack,
            width,
            height,
          ),
          _buildCategoryTile(
            context,
            languageProvider.translate('bags.womensBags'),
            Icons.shopping_bag,
            width,
            height,
          ),
          _buildCategoryTile(
            context,
            languageProvider.translate('bags.travelBags'),
            Icons.flight,
            width,
            height,
          ),
          _buildCategoryTile(
            context,
            languageProvider.translate('bags.other'),
            Icons.category,
            width,
            height,
          ),
...
class _BagsState extends State<Bags> {
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return Padding(
            ...
            child: FilterChip(
              label: Text(
                languageProvider.translate('bags.${category.name}'),
                style: GoogleFonts.cairo(
                  ...
                  ? StreamBuilder<QuerySnapshot>(
                    stream:
                        FirebaseFirestore.instance
                            .collection('stores')
                            .where('categories', arrayContains: 'Bags')
                            .snapshots(),
                    builder: (context, snapshot) {
                      if (snapshot.hasError) {
                        return Center(
                          child: Text(
                            languageProvider.translate('bags.errorLoading'),
                          ),
                        );
                      }
  ...
  Stream<QuerySnapshot> _buildProductsStream() {
    if (selectedCategory == 'other') {
      final excludedTypes = ['backpacks', 'womensBags', 'travelBags'];
            ...
            Expanded(
              child: StreamBuilder<QuerySnapshot>(
                ...
                builder: (context, snapshot) {
                  if (snapshot.hasError) {
                    return Center(
                      child: Text(LanguageProvider().translate('bags.error'), style: GoogleFonts.cairo()),
                    );
                  }
